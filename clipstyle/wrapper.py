...
class GeneratorWrapper(nn.Module):
    ...
    def __init__(self):
        ...
        G = Generator(z_dim=512, w_dim=512, mapping_kwargs={'num_layers': 8},
                      synthesis_kwargs={'channel_base': 32768,
                                        'channel_max': 512,
                                        'num_fp16_res': 4,
                                        'conv_clamp': 256},
                      c_dim=0, img_resolution=1024, img_channels=3).eval().requires_grad_(False)
        thisdir = os.path.dirname(__file__)
        G.load_state_dict(torch.load(os.path.join(thisdir, 'pretrained/ffhq_statedict.pt')))
